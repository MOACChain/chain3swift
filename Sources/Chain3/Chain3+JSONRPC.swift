...
public struct JsonRpcResponse: Decodable {
    ...
    public func response() throws -> DictionaryReader {
        if let error = error {
            throw Chain3Error.nodeError(error.message)
        } else if let result = result {
            return DictionaryReader(result)
        } else {
            throw Chain3Error.nodeError("No response found")
        }
    }
    ...
    public init(from decoder: Decoder) throws {
        ...
        if let rawValue = try? container.decodeIfPresent(String.self, forKey: .result) {
            ...
            result = rawValue
        } else if let rawValue = try? container.decodeIfPresent(SCSBlock.self, forKey: .result) {
            result = rawValue
