...
extension Chain3Options: Decodable {
...
extension MOACTransaction: Decodable {
    ...
    /// - Parameter decoder: The decoder to read data from.
    public init(from decoder: Decoder) throws {
        let options = try Chain3Options(from: decoder)
        let container = try decoder.container(keyedBy: CodingKeys.self)
        ...
        guard let r = try decodeHexToBigUInt(container, key: .r) else { throw Chain3Error.dataError }
        self.r = r

        guard let s = try decodeHexToBigUInt(container, key: .s) else { throw Chain3Error.dataError }
        self.s = s
...
public struct TransactionReceipt: Decodable {
    ...
    public init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        guard let blockNumber = try decodeHexToBigUInt(container, key: .blockNumber) else { throw Chain3Error.dataError }
        self.blockNumber = blockNumber

        guard let blockHash = try decodeHexToData(container, key: .blockHash) else { throw Chain3Error.dataError }
        self.blockHash = blockHash

        guard let transactionIndex = try decodeHexToBigUInt(container, key: .transactionIndex) else { throw Chain3Error.dataError }
        self.transactionIndex = transactionIndex

        guard let transactionHash = try decodeHexToData(container, key: .transactionHash) else { throw Chain3Error.dataError }
        ...
        guard let cumulativeGasUsed = try decodeHexToBigUInt(container, key: .cumulativeGasUsed) else { throw Chain3Error.dataError }
        self.cumulativeGasUsed = cumulativeGasUsed

        guard let gasUsed = try decodeHexToBigUInt(container, key: .gasUsed) else { throw Chain3Error.dataError }
...
//    transactionIndex = 0x99;
    ...
    public init(from decoder: Decoder) throws {
        ...
        guard let data = try decodeHexToData(container, key: .data) else { throw Chain3Error.dataError }
        self.data = data

        guard let logIndex = try decodeHexToBigUInt(container, key: .logIndex) else { throw Chain3Error.dataError }
        ...
        for top in topicsStrings {
            guard let topic = Data.fromHex(top) else { throw Chain3Error.dataError }
...
public struct Block: Decodable {
    ...
    public init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        guard let number = try decodeHexToBigUInt(container, key: .number) else { throw Chain3Error.dataError }
        self.number = number

        guard let hash = try decodeHexToData(container, key: .hash) else { throw Chain3Error.dataError }
        self.hash = hash

        guard let parentHash = try decodeHexToData(container, key: .parentHash) else { throw Chain3Error.dataError }
        self.parentHash = parentHash

        let nonce = try decodeHexToData(container, key: .nonce, allowOptional: true)
        self.nonce = nonce

        guard let sha3Uncles = try decodeHexToData(container, key: .sha3Uncles) else { throw Chain3Error.dataError }
        ...
        guard let transactionsRoot = try decodeHexToData(container, key: .transactionsRoot) else { throw Chain3Error.dataError }
        self.transactionsRoot = transactionsRoot

        guard let stateRoot = try decodeHexToData(container, key: .stateRoot) else { throw Chain3Error.dataError }
        self.stateRoot = stateRoot

        guard let receiptsRoot = try decodeHexToData(container, key: .receiptsRoot) else { throw Chain3Error.dataError }
        self.receiptsRoot = receiptsRoot

        if let minerAddress = try? container.decode(String.self, forKey: .miner) {
            guard minerAddress.isAddress else { throw Chain3Error.dataError }
            miner = Address(minerAddress)
        }

        guard let difficulty = try decodeHexToBigUInt(container, key: .difficulty) else { throw Chain3Error.dataError }
        self.difficulty = difficulty

        guard let totalDifficulty = try decodeHexToBigUInt(container, key: .totalDifficulty) else { throw Chain3Error.dataError }
        self.totalDifficulty = totalDifficulty

        guard let extraData = try decodeHexToData(container, key: .extraData) else { throw Chain3Error.dataError }
        self.extraData = extraData

        guard let size = try decodeHexToBigUInt(container, key: .size) else { throw Chain3Error.dataError }
        self.size = size

        guard let gasLimit = try decodeHexToBigUInt(container, key: .gasLimit) else { throw Chain3Error.dataError }
        self.gasLimit = gasLimit
        ...
        guard let timestampInt = UInt64(timestampString, radix: 16) else { throw Chain3Error.dataError }
        ...
        for str in unclesStrings {
            guard let d = Data.fromHex(str) else { throw Chain3Error.dataError }
            uncles.append(d)
        }
        self.uncles = uncles
    }
